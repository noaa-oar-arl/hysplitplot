...
class MapBox:
    ...
    def __init__(self, **kwargs):
        # Within MapBox, longitudes and latitudes use ranges [0, 360) and
        # [-90, 90], respectively.
        self.hit_map = None
    ...
    def set_ring_extent(self, settings):
        ...
        # set spatial extent of the rings
        half = int(kspan)
        for j in range(max(self._j-half, 0), min(self._j+half+1, self.sz[1])):
            self.hit_map[self._i, j] += 1
            self.hit_count += 1
        for i in range(max(self._i-half, 0), min(self._i+half+1, self.sz[0])):
            self.hit_map[i, self._j] += 1
            self.hit_count += 1
