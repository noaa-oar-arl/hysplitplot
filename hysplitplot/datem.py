...
class DatemReader(io.FormattedTextFileReader):
    ...
    def read(self, filename):
        ...
        while self.has_next():
            ...
            hour = int(v[3] / 100)
            minute = v[3] - hour * 100
            starting_datetime = datetime.datetime(v[0], v[1], v[2], hour, minute, 0, 0, time_zone)
            
            hour = int(v[4] / 100)
            minute = v[4] - hour * 100
            ending_datetime = starting_datetime + datetime.timedelta(hours=hour, minutes=minute)
            
            entry = Datem.Item(starting_datetime, ending_datetime, v[6], v[5], v[7], v[8])
